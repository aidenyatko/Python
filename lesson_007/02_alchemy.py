...
class Water:
    ...
    def __add__(self, other):
        if type(other) == type(Air()):
            return Storm()
        if type(other) == type(Fire()):
            return Steam()
        if type(other) == type(Earth()):
            return Mud()
...
print('')
print(Dust(), '+', Fire(), '=', Dust() + Fire())

# Усложненное задание (делать по желанию)
# Добавить еще элемент в игру.
# Придумать, что будет при сложении существующих элементов с новым.
#
# Acid_Rain	= Rain() + Smoke() / Rain() + Smog()
# Airplane = Bird() + Steel() / Bird() + Metal()
# Alarm_Clock() = Clock() + Sound()
